...
class BasicAtomicMutableRawPointerTests: XCTestCase {
  private let _mraw1 = UnsafeMutableRawPointer.allocate(
    byteCount: 16, alignment: 1)
  private let _mraw2 = UnsafeMutableRawPointer.allocate(
    byteCount: 16, alignment: 1)
  ...
  func test_exchange_sequentiallyConsistent() {
    ...
    XCTAssertEqual(
      v.exchange(_mraw1, ordering: .sequentiallyConsistent), _mraw1)
    XCTAssertEqual(v.load(ordering: .relaxed), _mraw1)

    XCTAssertEqual(
      v.exchange(_mraw2, ordering: .sequentiallyConsistent), _mraw1)
    XCTAssertEqual(v.load(ordering: .relaxed), _mraw2)

    XCTAssertEqual(
      v.exchange(_mraw2, ordering: .sequentiallyConsistent), _mraw2)
    XCTAssertEqual(v.load(ordering: .relaxed), _mraw2)
  }
  ...
  public static var allTests:
    [(String, (BasicAtomicMutableRawPointerTests) -> () -> Void)] = [
