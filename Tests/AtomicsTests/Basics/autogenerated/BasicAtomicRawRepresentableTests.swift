...
class BasicAtomicRawRepresentableTests: XCTestCase {
  ...
  func test_exchange_acquiringAndReleasing() {
    ...
    XCTAssertEqual(
      v.exchange(Fred.one, ordering: .acquiringAndReleasing), Fred.one)
    XCTAssertEqual(v.load(ordering: .relaxed), Fred.one)

    XCTAssertEqual(
      v.exchange(Fred.two, ordering: .acquiringAndReleasing), Fred.one)
    XCTAssertEqual(v.load(ordering: .relaxed), Fred.two)

    XCTAssertEqual(
      v.exchange(Fred.two, ordering: .acquiringAndReleasing), Fred.two)
    XCTAssertEqual(v.load(ordering: .relaxed), Fred.two)
  }
  ...
  func test_exchange_sequentiallyConsistent() {
    ...
    XCTAssertEqual(
      v.exchange(Fred.one, ordering: .sequentiallyConsistent), Fred.one)
    XCTAssertEqual(v.load(ordering: .relaxed), Fred.one)

    XCTAssertEqual(
      v.exchange(Fred.two, ordering: .sequentiallyConsistent), Fred.one)
    XCTAssertEqual(v.load(ordering: .relaxed), Fred.two)

    XCTAssertEqual(
      v.exchange(Fred.two, ordering: .sequentiallyConsistent), Fred.two)
    XCTAssertEqual(v.load(ordering: .relaxed), Fred.two)
  }
  ...
  public static var allTests:
    [(String, (BasicAtomicRawRepresentableTests) -> () -> Void)] = [
