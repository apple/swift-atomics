...
The current version of the `Atomics` module does not implement APIs for tagged atomics (see issue #5), although it does expose a `DoubleWord` type that can be used to implement them. (Atomic strong references are already implemented in terms of `DoubleWord`, although in their current form they do not expose any user-customizable bits.)
